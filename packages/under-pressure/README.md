...
```
npm add @honohub/under-pressure
```
...
- _routeOpts_ - Any Fastify [route options](https://fastify.dev/docs/latest/Reference/Routes/#routes-options) except `schema`
- _routeSchemaOpts_ - As per the Fastify route options, an object containing the schema for request
- _routeResponseSchemaOpts_ - An object containing the schema for additional response items to be merged with the default response schema, see below
- _url_ - The URL to expose the status route on
...
The `@honohub/under-pressure` project is heavily inspired by [@fastify/under-pressure](https://github.com/fastify/under-pressure)
